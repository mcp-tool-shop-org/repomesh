...
The CLI also ships as `ghcr.io/mcp-tool-shop-org/repomesh`, tagged with the npm version. It runs as a non-root user. A wallet seed is not in the image.

```bash
docker run --rm ghcr.io/mcp-tool-shop-org/repomesh verify-anchor --tx <TX_HASH>
```

The daily anchor job builds this image from the checkout and posts with it. The network in `anchor/xrpl/config.json` is testnet. A mainnet post needs a funded account whose classic address is already in the shipped allowlist. That list is a ceiling: a fetched config can drop an account and cannot add one.
